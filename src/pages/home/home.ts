...
export class HomePage {
  ...
  updateKaoDOM() {
  	let textShadow = "";
  	if (this.currentKao.shadowLength < 0) {
  		this.currentKao.shadowLength = 0;
  	} else if (this.currentKao.shadowLength > 20) {
  		this.currentKao.shadowLength = 20;
  	} else { }
  	this.currentKao.shadowLength = Math.floor(this.currentKao.shadowLength);
  	if (this.currentKao.patternId >= this.backgroundDefs.length) {
  		this.currentKao.patternId = this.backgroundDefs.length - 1;
  	} else if (this.currentKao.patternId >= -1 && this.currentKao.patternId < this.backgroundDefs.length) {
  		// leave it
		// had to do it this weird way to account for NaN giving me trouble
  	} else {
  		this.currentKao.patternId = -1
  	}
  	this.currentKao.patternId = Math.floor(this.currentKao.patternId);
